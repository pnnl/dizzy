...
installable uv package). Requires that `def/` stubs exist; run `generate definitions` first.

Also emits runtime-neutral JSON Schema into `gen_schema/` for whatever the
`json_schema` section of `libconfig.yaml` asks for:

```yaml
json_schema:
  contracts: [commands, queries]   # any of: commands | events | queries | models
  output_dir: gen_schema           # optional; this is the default
```

One document per `def/` source, so `gen_schema/commands.schema.json` carries every
command under `$defs` and `gen_schema/queries/get_recipe.schema.json` carries both
`GetRecipeInput` and `GetRecipeOutput`. Validate one payload by pointing a validator at
`{"$ref": "#/$defs/<ClassName>"}`. Drop the section from `libconfig.yaml` to stop
emitting them; a libconfig without the section emits nothing.
...
fill in. Requires that `libconfig.yaml` exists; run `generate definitions` first. The Python path is the
...
## What You Author After `generate definitions`
...
## What You Implement After `generate libraries`
...
```
<output_dir>/
├── def/                              # YOUR LinkML schemas (authored after `generate definitions`)
│   ├── commands.yaml
│   ├── events.yaml
│   ├── environment.yaml              #   only when the feat declares `environment:`
│   ├── telemetry.yaml                #   only when the feat declares `telemetry:`
│   ├── models/<name>.yaml
│   └── queries/<name>.yaml
├── libconfig.yaml                    # YOUR runtime assignments (authored after `generate definitions`)
├── gen_schema/                       # runtime-neutral JSON Schema (generated by `generate static`)
│   ├── commands.schema.json          #   opt in via libconfig's `json_schema:` section
│   ├── events.schema.json
│   ├── models/<name>.schema.json
│   └── queries/<name>.schema.json
└── lib/                              # one workspace per runtime (self-contained but for wiring/)
    └── python-uv/
        ├── pyproject.toml            # uv workspace (lists every member below)
        ├── gen_def/                  # installable package (generated by `generate static`)
        │   ├── pyproject.toml
        │   └── gen_def/              # import root → `gen_def`
        │       ├── pydantic/{commands,events}.py, models/<name>.py, query/<name>.py
        │       └── sqla/models/<name>.py
        ├── gen_int/                  # installable package (generated by `generate static`)
        │   ├── pyproject.toml
        ...
        ├── wiring/                   # elements bound to the engine (generated by `generate wiring`)
        │   ├── pyproject.toml        #   the only generated package that depends on DIZZY
        │   └── src/
        │       ├── wiring.py         #   build_engine / build_runtime / host_app — never edit
        │       └── <feat>.feat.yaml  #   the feature-file it was generated from, carried along
...
- `lib/python-uv/wiring/` — auto-generated binding from elements to a `dizzy.engine`
  engine, plus the `HostApp` a shell resolves. The only generated package that depends on
  DIZZY, so the workspace root also carries a `[tool.uv.sources]` entry naming where DIZZY
  comes from (`--dizzy-source` overrides it)
- `gen_schema/` — auto-generated JSON Schema, one document per `def/` source; language-neutral
  by design, so it lives beside `lib/` instead of inside a runtime tree
